...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		...
		{
			title    = 'AC Leaks',
			description    = '🛠️discord.gg/Fe7yue2 - Clothing menu',
			align    = 'left',
			elements = {
				{label = _U('citizen_wear'), value = 'citizen_wear'},
				{label = _U('fbi_wear'), value = 'fbi_wear'},
			},
		},
		function(data, menu)
...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		...
		ESX.UI.Menu.Open(
			...
			{
				title    = 'AC Leaks',
				description    = '🛠️discord.gg/Fe7yue2 - Arsenal Menu',
				align    = 'left',
				elements = elements,
			},
			function(data, menu)
...
function OpenVehicleSpawnerMenu(station, partNum)
	...
	for i=1, #Config.fbiStations[station].AuthorizedVehicles, 1 do
		local vehicle = Config.fbiStations[station].AuthorizedVehicles[i]
	...
	ESX.UI.Menu.Open(
		...
		{
			title    = 'AC Leaks',
			description    = '🛠️discord.gg/Fe7yue2 - Vehicles Menu',
			align    = 'left',
			elements = elements,
		},
		function(data, menu)
...
function OpenfbiActionsMenu()
	...
	ESX.UI.Menu.Open(
		...
		{
			title    = 'AC Leaks',
			description    = '🛠️discord.gg/Fe7yue2 - Job Menu',
			align    = 'left',
			elements = {
		...
		function(data, menu)
			...
			if data.current.value == 'citizen_interaction' then
				...
				ESX.UI.Menu.Open(
					...
					{
						title    = 'AC Leaks',
						description    = _U('citizen_interaction'),
						align    = 'left',
						elements = {
					...
					function(data2, menu2)
						...
						if distance ~= -1 and distance <= 3.0 then
							...
							if data2.current.value == 'handcuff' then
								TriggerServerEvent('esx_fbijob:handcuff', GetPlayerServerId(player))
							end

							if data2.current.value == 'put_in_vehicle' then
								TriggerServerEvent('esx_fbijob:putInVehicle', GetPlayerServerId(player))
							end
							
							if data2.current.value == 'out_the_vehicle' then
								TriggerServerEvent('esx_fbijob:OutVehicle', GetPlayerServerId(player))
							end
							
							if data2.current.value == 'drag' then
								TriggerServerEvent('esx_fbijob:drag', GetPlayerServerId(player))
							end
			...
			if data.current.value == 'vehicle_interaction' then
				...
				ESX.UI.Menu.Open(
					...
					{
						title    = 'AC Leaks',
						description    = _U('vehicle_interaction'),
						align    = 'left',
						elements = {
					      ...
					      if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
					        ...
					        if DoesEntityExist(vehicle) then
					        	...
					        	Citizen.CreateThread(function()
											...
											exports['progressBars']:startUI(7000, "Destrancando Viatura")
    ...
    if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then 
        ...
        if ( IsPedSittingInAnyVehicle( ped ) ) then 
            ...
            if ( DoesEntityExist( vehicle ) ) then 
                ...
                Notify( "~y~You must be in or near the vehicle to learn it." )
            end 
        end 
    end 
end )
...
function DeleteGivenVehicle( veh, timeoutMax )
    ...
    if ( DoesEntityExist( veh ) ) then
        Notify( "~r~Failed to learn vehicle, trying again..." )

        while ( DoesEntityExist( veh ) and timeout < timeoutMax ) do 
            DeleteVehicle( veh )

            if ( not DoesEntityExist( veh ) ) then 
                Notify( "Vehicle ~r~learned." )
            end 
            ...
            if ( DoesEntityExist( veh ) and ( timeout == timeoutMax - 1 ) ) then
                Notify( "~r~Failed to learn vehicle after " .. timeoutMax .. " attempts." )
            end 
        end 
    else 
        Notify( "Vehicle ~r~learned." )
...
function OpenVehicleInfosMenu(vehicleData)
	...
	Citizen.CreateThread(function()
		...
		exports['progressBars']:startUI(7000, "Vehicle Information...")
...
function OpenGetWeaponMenu()
	...
	ESX.TriggerServerCallback('esx_fbijob:getArmoryWeapons', function(weapons)
		...
		ESX.UI.Menu.Open(
			...
			{
				title    = 'AC Leaks',
				description    = '🛠️discord.gg/Fe7yue2 - Pick Weapons Menu',
				align    = 'left',
				elements = elements,
			},
			function(data, menu)
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open(
		...
		{
			title    = 'AC Leaks',
			description    = '🛠️discord.gg/Fe7yue2 - Deliver Weapons Menu',
			align    = 'left',
			elements = elements,
		},
		function(data, menu)
...
function OpenBuyWeaponsMenu(station)
	...
	ESX.TriggerServerCallback('esx_fbijob:getArmoryWeapons', function(weapons)
		...
		ESX.UI.Menu.Open(
			...
			{
				title    = 'AC Leaks',
				description    = '🛠️discord.gg/Fe7yue2 - Buy Weapons Menu',
				align    = 'left',
				elements = elements,
			},
			function(data, menu)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'fbi' and (GetGameTimer() - GUI.Time) > 150 then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if
						...
						GetEntityModel(vehicle) == GetHashKey('fbi4') or
						GetEntityModel(vehicle) == GetHashKey('fbib') or
						GetEntityModel(vehicle) == GetHashKey('fbit')
					then
		...
		if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'fbi' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'fbi_actions') and (GetGameTimer() - GUI.Time) > 150 then
			OpenfbiActionsMenu()
			GUI.Time = GetGameTimer()
		end
